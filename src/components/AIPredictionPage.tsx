...
const AIPredictionPage: React.FC<Props> = ({ data, predictionResult, symbolName, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-neutral-950 pb-12 text-neutral-100">
      <header className="bg-neutral-900 border-b border-neutral-800 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">AI Price Prediction Dashboard</h1>
              <p className="text-xs text-neutral-400 font-medium uppercase tracking-wider">{symbolName || 'Asset'} • Ensemble Forecasting</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {!result && !error ? (
          <div className="max-w-2xl mx-auto mt-20 bg-neutral-900 p-8 rounded-3xl shadow-sm border border-neutral-800">
          ...
          <div className="max-w-xl mx-auto mt-20 text-center space-y-4">
            ...
            <h2 className="text-xl font-bold text-white">Prediction Failed</h2>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              {result.marketContext && (
                <div className="bg-neutral-900 p-6 rounded-3xl shadow-sm border border-neutral-800">
                  ...
                  <div className={`text-xl font-bold ${result.marketContext.trend === 'Bullish' ? 'text-emerald-500' : result.marketContext.trend === 'Bearish' ? 'text-red-500' : 'text-neutral-400'}`}>
                    {result.marketContext.trend} <span className="text-sm font-normal text-neutral-600 ml-2">({result.marketContext.dailyReturn > 0 ? '+' : ''}{result.marketContext.dailyReturn.toFixed(2)}%)</span>
                  </div>
                </div>
              )}
              ...
              {result.institutionalFlow && (
                <div className="bg-neutral-900 p-6 rounded-3xl shadow-sm border border-neutral-800">
                  ...
                  <div className={`text-xl font-bold flex items-center gap-2 ${result.institutionalFlow.signal === 'Bullish' ? 'text-emerald-500' : result.institutionalFlow.signal === 'Bearish' ? 'text-red-500' : 'text-yellow-500'}`}>
                    {result.institutionalFlow.signal} <span className="text-sm text-neutral-600 font-normal ml-2">({result.institutionalFlow.score}/100)</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-neutral-900 p-6 rounded-3xl shadow-sm border border-neutral-800">
                <div className="text-sm font-medium text-neutral-500 mb-1">Expected Trend</div>
              ...
              <div className="bg-neutral-900 p-6 rounded-3xl shadow-sm border border-neutral-800 relative group">
                ...
                <div className="text-xl font-bold text-white flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-neutral-800 rounded-full h-2">
                    <div className="bg-purple-500 h-2 rounded-full" style={{ width: `${result.confidenceScore || 0}%` }} />
                  </div>
                </div>
              </div>
              <div className="bg-neutral-900 p-6 rounded-3xl shadow-sm border border-neutral-800">
                <div className="text-sm font-medium text-neutral-500 mb-1">Risk Level</div>
            ...
            <div className="bg-neutral-900 p-6 rounded-3xl shadow-sm border border-neutral-800">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="text-sm text-neutral-400 font-medium bg-neutral-800/50 px-3 py-1.5 rounded-lg border border-neutral-700">
                  Projected Range: <span className="text-white font-bold">{renderValue(result.projectedRange?.min, 0, '₹')} – {renderValue(result.projectedRange?.max, 0, '₹')}</span>
                </div>
              </div>
                ...
                <ResponsiveContainer>
                  <ComposedChart data={chartData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                    ...
                    <Tooltip
                      ...
                      contentStyle={{ backgroundColor: '#171717', borderRadius: '12px', border: '1px solid #262626', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.5)' }}
                      labelStyle={{ color: '#f5f5f5', fontWeight: 'bold', borderBottom: '1px solid #262626', paddingBottom: '8px', marginBottom: '8px' }}
                  ...
                  <div className="bg-neutral-900 p-6 rounded-3xl shadow-sm border border-neutral-800">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <label className="flex items-center gap-2 text-sm text-neutral-400 cursor-pointer">
                        <input
                          type="checkbox"
                          className="rounded border-neutral-700 bg-neutral-900 text-purple-600 focus:ring-purple-500/50 cursor-pointer"
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-center bg-neutral-950/50 p-3 rounded-xl border border-neutral-800">
                        ...
                        <span className="font-bold text-white">{renderValue(result.backtestMetrics.mae, 2, '₹')}</span>
                      </div>
                      <div className="flex justify-between items-center bg-neutral-950/50 p-3 rounded-xl border border-neutral-800">
                        <span className="text-sm text-neutral-400 font-medium">Deviation (RMSE)</span>
                        <span className="font-bold text-white">{renderValue(result.backtestMetrics.rmse, 2, '₹')}</span>
                      </div>
                    </div>
                ...
                {result.newsSentiment && (
                  <div className="bg-neutral-900 p-6 rounded-3xl shadow-sm border border-neutral-800">
                    ...
                    <div className="flex items-center gap-3 mb-6">
                      <div className={`px-3 py-1.5 rounded-lg font-bold text-sm border ${result.newsSentiment?.label === 'Positive' ? 'bg-emerald-950/30 text-emerald-400 border-emerald-900/50' :
                        result.newsSentiment?.label === 'Negative' ? 'bg-red-950/30 text-red-400 border-red-900/50' :
                ...
                <div className="bg-neutral-900 p-8 rounded-3xl shadow-sm border border-neutral-800 flex-grow">
                  ...
                  <div className="prose prose-invert prose-purple max-w-none">
                    {(Array.isArray(result.reasoning) ? result.reasoning : String(result.reasoning || '').split('\n'))
                      ...
                      .map((paragraph, i) => (
                        <p key={i} className="text-neutral-300 leading-relaxed text-lg">{paragraph}</p>
                      ))}
                  </div>
                  ...
                  <ul className="space-y-3">
                    {(Array.isArray(result.keyFactors) ? result.keyFactors : (result.keyFactors ? [String(result.keyFactors)] : [])).map((f, i) => (
                      <li key={i} className="flex items-start gap-3 bg-neutral-950/50 p-4 rounded-2xl text-sm leading-relaxed text-neutral-300 border border-neutral-800">
                        <div className="w-6 h-6 rounded-full bg-purple-900/40 text-purple-400 flex items-center justify-center shrink-0 font-bold text-xs mt-0.5 border border-purple-500/20">{i + 1}</div>
                        <span>{f}</span>
                      </li>
                    ))}
                  </ul>
